...
class Model(object):
    ...
    def __init__(self, model_parameters, initial_state, executer):
        ...
        assert len(self.microphysics['r_min']) == len(
            self.microphysics['particle_count'])
        assert len(self.microphysics['r_min']) == len(self._initial_state.qc)
        self.executer = executer(step, self, initial_state, int(
            self.dt_output / float(self.dt)))
...
def nucleation_slice(state, microphysics):
    ...
    m = (S < bf.critical_super_saturation(r_min, state.T)) & (
        np.isclose(bf.radius(state.qc, particle_count, r_min), r_min))
    return np.invert(m)


def step(model, old_state, math=np):
    new_state = model.prepare_new_state(old_state, math=math)
    delta_Ts, delta_qvs, delta_qc, delta_T_E, delta_age = model.calculate_tendencies(
        new_state, math=math)
    new_state.qc = new_state.qc + delta_qc
    new_state.T += math.sum(delta_Ts) + delta_T_E
    new_state.qv += math.sum(delta_qvs)
    new_state.z = new_state.z + bf.fall_speed(bf.radius(
        new_state.qc, model.microphysics['particle_count'], model.microphysics['r_min'])) * model.dt
